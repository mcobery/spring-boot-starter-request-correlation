...
/**
 * The request correlation properties. These properties can be in the application.properties file to
 * override the default behavior of this starter.
 *
...
public class RequestCorrelationProperties {
  /**
   * The priority order of the filter. Defaults to Ordered.HIGHEST_PRECEDENCE, so the correlating id
   * is set very early in the process, but you may need to override this. For example, if you use
   * shared sessions via the spring-session project, you need to make sure the request correlation
   * filter comes AFTER the Spring Session filter, or the request correlation filter won't get the
   * right request object and your ids will be wrong.
   */
  private int filterOrder = Ordered.HIGHEST_PRECEDENCE;

  /**
   * The starting point of the filter order property. This value will be added to the filter-order
   * property to get the actual order. Defaults to "zero" so that using filter-order by itself gives
   * predictable results. If you want the filter order to be an offset from the Highest Precedence,
   * set the filter-order to the offset, and filter-order-from to "highest_precedence"
   */
